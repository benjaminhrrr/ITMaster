...
def leer_datos(cartel, **arg):
    ...
    for i, (valor, tipo) in enumerate(zip(valores, arg.values())):
        ...
        try:
            ...
            print(
                f"No se pudo convertir el valor '{valor}' al tipo '{tipo.__name__}' en la posición {i+1}."
            )
            return None
    return lista
...
def leer_numero(
    mensaje: str = "Ingrese un número",
    minimo=-float("inf"),
    maximo=float("inf"),
    tipo: type[int | float] = int,
):
    if tipo is int:
...
def leer_entero(
    mensaje: str = "Ingrese un entero: ",
    minimo: int = -float("inf"),
    maximo: int = float("inf"),
) -> int:
    """
...
# En esta línea estaba la función leer_entero_rango, ahora valida el rango
# leer_entero

# En esta línea estaba la función leer_float_rango,ahora valida el rango
# leer_float
...
def leer_datos(cartel: str, **arg) -> tuple:
    ...
    for nombre_dato, tipo in arg.items():
        try:
            ...
            raise ValueError(
                f"No se pudo convertir el valor {valor} al tipo {tipo.__name__}.\n {e}"
            )
